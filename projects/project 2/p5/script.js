...
class TaskManager {
    ...
    editTask(taskId) {
        ...
        this.showTasks();
        const currentDiv = document.querySelector("div[contenteditable='true']");
        currentDiv.focus();
    }
    ...
    showTasks() {
        ...
        this.tasks.forEach(t => {
            ...
            div.innerHTML = `
                <h3>${t.title}</h3>
                <p><b>Creative time:</b> ${t.addedTime}</p>
                <p><b>Desctription:</b><div class"onEdit" contenteditable="${t.contentEditable}"> ${t.description || '*No comment*'}</div></p>
                <footer>
                    <button class="remove">Delete</button>
                    ${t.isCompleted ? '<button class="uncomplete">Not carried out</button>' : '<button class="complete">Done</button>'}
                    <button class="edit">Edit</button>
                    <button class="finishEdit">Save</button>
                </footer>
            `;
